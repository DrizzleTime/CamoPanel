...
export function CopilotPage() {
  ...
  const loadConfigStatus = async () => {
    ...
    try {
      const response = await apiRequest<CopilotConfigStatus>("/api/copilot/config");
  ...
  const loadProviders = async () => {
    ...
    try {
      const response = await apiRequest<{ items: CopilotProvider[] }>("/api/copilot/providers");
  ...
  const saveProvider = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingProvider) {
        await apiRequest(`/api/copilot/providers/${editingProvider.id}`, {
          method: "PUT",
          body: JSON.stringify({
            ...values,
            type: "openai",
        ...
        await apiRequest("/api/copilot/providers", {
          method: "POST",
          body: JSON.stringify({
            ...values,
            type: "openai",
  ...
  const deleteProvider = async (provider: CopilotProvider) => {
    try {
      await apiRequest(`/api/copilot/providers/${provider.id}`, { method: "DELETE" });
  ...
  const saveModel = async () => {
    ...
    try {
      ...
      setSubmitting(true);
      if (editingModel) {
        await apiRequest(`/api/copilot/models/${editingModel.id}`, {
          method: "PUT",
          body: JSON.stringify(values),
        });
        message.success("模型已更新");
      } else {
        await apiRequest(`/api/copilot/providers/${activeProvider.id}/models`, {
          method: "POST",
          body: JSON.stringify(values),
        });
  ...
  const deleteModel = async (aiModel: CopilotModel) => {
    try {
      await apiRequest(`/api/copilot/models/${aiModel.id}`, { method: "DELETE" });
  ...
  const setDefaultModel = async (aiModel: CopilotModel) => {
    try {
      await apiRequest(`/api/copilot/models/${aiModel.id}`, {
        method: "PUT",
        body: JSON.stringify({
          name: aiModel.name,
          enabled: aiModel.enabled,
          ...
          <Card className="glass-card">
            <Space.Compact style={{ width: "100%" }}>
              ...
              <Button
                ...
                icon={<SendOutlined />}
                loading={sending}
                disabled={!configStatus?.configured || !session}
                onClick={() => void send()}
              >
                发送
              </Button>
